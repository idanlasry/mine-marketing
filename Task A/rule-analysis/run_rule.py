...
# Turn-off:   -(adset's average daily profit over the 3 days before the action day) x days left in the data.
#             Zero-spend days in the window count as $0. No spend in the window (day 1) -> no value, flagged.
#             No days left (last data day) -> $0.
...
hist AS (
  SELECT d.adset_id, d.dt, NULLIF(COUNTIF(p.spend > 0), 0) AS history_days,
         AVG(IFNULL(p.revenue, 0) - p.spend) AS avg_daily_profit
...
j AS (
  SELECT d.adset_id, d.dt, d.action, d.live, d.last_set, p.spend AS day_spend,
         ...
         h.history_days, h.avg_daily_profit, DATE_DIFF(ld.last_dt, d.dt, DAY) AS days_left, ld.first_dt
  FROM d CROSS JOIN last_day ld
  JOIN $.performance_scoped p ON p.adset_id = d.adset_id AND CAST(p.date AS DATE) = d.dt
...
SELECT adset_id, dt, action,
       ...
       history_days, ROUND(avg_daily_profit, 2) AS avg_daily_profit, days_left,
       ROUND(CASE
         WHEN action LIKE 'Decrease%' THEN IF(day_spend >= 0.95 * last_set, -roi_close * (live - last_set), 0)
         WHEN action = 'Turn OFF' AND days_left = 0 THEN 0
         WHEN action = 'Turn OFF' AND history_days IS NULL THEN NULL
         WHEN action = 'Turn OFF' THEN -avg_daily_profit * days_left
       END, 2) AS value,
       CASE
         ...
         WHEN action = 'Turn OFF' AND days_left = 0 THEN 'off: last data day -> $0'
         WHEN action = 'Turn OFF' AND history_days IS NULL AND DATE_SUB(dt, INTERVAL 1 DAY) < first_dt
              THEN 'off: last 3 days are before the data starts -> no value'
         WHEN action = 'Turn OFF' AND history_days IS NULL THEN 'off: no spend in last 3 days -> no value'
         WHEN action = 'Turn OFF' AND DATE_SUB(dt, INTERVAL 3 DAY) < first_dt
              THEN 'off: last-3-days avg x days left (window partly before data)'
         WHEN action = 'Turn OFF' THEN 'off: last-3-days avg profit x days left'
         ELSE 'not valued'
       END AS how
...
if v is not None and len(v):
    print()
    print(f"TOTAL value: {v['value'].sum():+.2f}  |  counted {v['value'].notna().sum()} of {len(v)} decisions"
          f"  |  too small to matter if |total| < 25.86")
